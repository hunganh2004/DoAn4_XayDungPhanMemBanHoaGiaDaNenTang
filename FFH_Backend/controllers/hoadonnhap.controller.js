...
import service from '../services/hoadonnhap.service.js'
...
class HoadonnhapController {
    ...
    insert(req, res) {
        const hoadonnhap = req.body
        service.insert(hoadonnhap, (result) => {
            res.send(result)
        })
    }
    update(req, res) {
        const hoadonnhap = req.body
        // const id = req.params.id
        service.update(hoadonnhap, (result) => {
            res.send(result)
        })
    }
    delete(req, res) {
...
export default new HoadonnhapController(service)